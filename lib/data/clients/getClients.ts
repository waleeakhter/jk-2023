...
const getClients = async (searchParams?: any[] | URLSearchParams) => {
    ...
    const res = await ClientModel.find(({ ...(id && { _id: id }), ...(client && { name: { $regex: client, $options: "i" } }) }))
    .sort({ name: 1 }).collation({ locale: "en" }).lean()
    const clients = res.length ? res.map((item) => ({ ...item, _id: (item._id as string).toString() })) as Client[] : []
